...
---
# UPDATE 2026-09-22 (evening) - "printer won't connect" was a race condition in link_usb.cpp, not the printer

**Symptom:** `POST /printer/connect` reliably returned "No CH340 serial chip found on the OTG port" - looked exactly like a wiring/cable/power problem, and burned a lot of time on that theory (checked cable, checked the printer's own power switch, tried a raw USB bus enumeration - it genuinely found 0 devices on the bus at the moment of failure). The user insisted the cable/printer/board were unchanged from a session earlier the same day that had worked (confirmed independently: the SD log's oldest surviving entries show a real print with real temperatures, `hot=220.0/220.0 bed=60.4/60.0` - not the simulator's flat 24.0 C, so it had genuinely connected and printed before).

**Root cause, found by watching live UART serial during a connect attempt (not visible from the SD log or the HTTP API alone):** `cdc_acm_dev_hdl_t dev` in `link_usb.cpp` was read/written from two contexts with no synchronization - our own `UsbLink::close()` (called from the engine/HTTP-handler context) and `onEvent()`'s `CDC_ACM_HOST_DEVICE_DISCONNECTED` handler (called from the USB host library's own event context on a real disconnect). If both raced, `cdc_acm_host_close()` could be called twice on the same handle; the second call hits an already-released interface deep in the vendored `usb_host_cdc_acm` component, which does `ESP_ERROR_CHECK` internally and **panics the whole board** (`abort()`, full reboot). Confirmed via serial: `ESP_ERROR_CHECK failed: ... at cdc_acm_host.c:699 ... cdc_acm_host_close`, immediately followed by `BOOT: PANIC (crash)`.

The failure loop this produced was self-obscuring: a connect attempt would actually succeed (`usb: CH34x 1a86:7523 opened`, confirmed once caught live on serial), then the very next close/reconnect (the phone's `AutoConnectThread` retries constantly while the plug is on) would race and crash the board - wiping the connected state and rebooting before any status poll ever caught the success. Every symptom pointed at "can't connect" when the board was actually connecting fine and then immediately crashing.
