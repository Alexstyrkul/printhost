...
static void in_xfer_cb(usb_transfer_t *transfer)
{
    ...
    if (cdc_dev->rx_paused) {
        return;  // PrintHost: polling is stopped on purpose; cdc_acm_host_rx_resume() restarts it
    }
    ESP_LOGD(TAG, "Submitting poll for BULK IN transfer");
    cdc_dev->rx_active = true;
...
/*
 * PrintHost addition: stop / restart the permanent bulk IN polling.
 * A device that has nothing to say NAKs every poll, which keeps the USB bus busy all the time; on this
 * board that traffic wrecks the 2.4 GHz Wi-Fi. While paused, data the device sends waits in its own
 * FIFO and is delivered as soon as polling resumes.
 */
esp_err_t cdc_acm_host_rx_pause(cdc_acm_dev_hdl_t cdc_hdl)
{
    cdc_dev_t *cdc_dev = (cdc_dev_t *)cdc_hdl;
    if (cdc_dev == NULL || cdc_dev->data.in_xfer == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
    if (cdc_dev->rx_paused) {
        return ESP_OK;
    }
    cdc_dev->rx_paused = true;  // in_xfer_cb() will not resubmit
    if (cdc_dev->rx_active) {
        // cancel the transfer that is pending right now: halt, flush (completes it as canceled), clear
        return cdc_acm_reset_transfer_endpoint(cdc_dev->dev_hdl, cdc_dev->data.in_xfer);
    }
    return ESP_OK;
}

esp_err_t cdc_acm_host_rx_resume(cdc_acm_dev_hdl_t cdc_hdl)
{
    cdc_dev_t *cdc_dev = (cdc_dev_t *)cdc_hdl;
    if (cdc_dev == NULL || cdc_dev->data.in_xfer == NULL) {
        return ESP_ERR_INVALID_ARG;
    ...
    cdc_dev->rx_paused = false;
    if (cdc_dev->rx_active) {
        return ESP_OK;  // a transfer is still winding down; its callback will resubmit
    }
    cdc_acm_reset_in_transfer(cdc_dev);
    cdc_dev->rx_active = true;
    esp_err_t err = usb_host_transfer_submit(cdc_dev->data.in_xfer);
    if (err != ESP_OK) {
        cdc_dev->rx_active = false;
    }
    return err;
